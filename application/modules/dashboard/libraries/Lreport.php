   ...
   public function expiry_date()
    {
        ...
        $expiry_date = $CI->Reports->expiry_date();
        ...
        $reportList = $CI->parser->parse('dashboard/report/expiry_date', $data, true);
        return $reportList;
    }
    ...
    public function stock_report_product_wise($product_id, $supplier_id, $from_date, $to_date, $links, $per_page, $page)
    {
		...
		//echo count($stok_report);die;
        if (count($stok_report) > 0) {
    ...
    public function stock_report_variant_wise($from_date = false, $to_date = false, $store_id = false, $links = false, $per_page = false, $page = false, $product_id = false)
    {
        ...
        if ($stok_report) {
            foreach ($stok_report as $k => $v) {
				...
				$CI->db->select('*');
				$CI->db->from('stock_adjustment_details sd');
				$CI->db->where('sd.product_id', @$v['product_id']);
				$CI->db->where('sd.variant_id', @$v['variant_id']);
				$CI->db->order_by("id", "desc");
				if (!empty($variant_color)) {
					$CI->db->where('sd.color_variant', @$v['variant_color']); // Change "or_where" to "where" here
				}
				$update_stock = $CI->db->get()->row();
                ...
                $stok_report[$k]['stok_quantity'] = abs($qty); //(@$purchase->totalPrhcsCtn + @$receive->totalReceive - $sales->totalSalesQnty + @$send->totalSend);
				//d($sales);
                $stok_report[$k]['SubTotalOut']   = ($sub_total_out + $sales->totalSalesQnty);
                $sub_total_out                    = $stok_report[$k]['SubTotalOut'];
                ...
                $stok_report[$k]['SubTotalStock'] = ($sub_total_stock + $stok_report[$k]['stok_quantity']);
                $sub_total_stock = $stok_report[$k]['stok_quantity'];
            }
