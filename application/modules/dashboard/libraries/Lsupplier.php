...
class Lsupplier {
	...
	public function supplier_edit_data($supplier_id)
	{
		...
		$data=array(
			...
			'cin' 		=> $supplier_detail[0]['cin'],
			'pan' 		=> $supplier_detail[0]['pan'],    
			'cfssai' 		=> $supplier_detail[0]['cfssai'],
			'sfssai' 		=> $supplier_detail[0]['sfssai'],
			'details' 		=> $supplier_detail[0]['details'],      
			'bank_account' 		=> $supplier_detail[0]['bank_account'],
			'account_no' 		=> $supplier_detail[0]['account_no'],
			'ifsc' 		=> $supplier_detail[0]['ifsc'],
			'account_type' 		=> $supplier_detail[0]['account_type'],
			'bank_name' 		=> $supplier_detail[0]['bank_name'],
			'branch' 		=> $supplier_detail[0]['branch'],
	...
	public function supplier_ledger($supplier_id)
	{
		...
		$previous_balance = $supplier_details[0]['previous_balance']; // Replace with your actual previous balance
		//d($previous_balance);die;
		$ledger 	= $CI->Suppliers->suppliers_ledger1($supplier_id);
		$summary 	= $CI->Suppliers->suppliers_transection_summary1($supplier_id);
			
		$balance = $previous_balance; // Set the initial balance to the previous_balance
		if(!empty($ledger)){
			foreach($ledger as $index=>$value){
				$ledger[$index]['final_date'] = $CI->occational->dateConvert($ledger[$index]['date']);
				//echo $balance-$ledger[$index]['amount'];die;
		...
		$data=array(
			...
			'previous_balance' 	=> $supplier_details[0]['previous_balance'],
			'ledger' 			=> $ledger,
			'total_credit'		=> number_format($summary[0][0]['total_credit'], 2, '.', ','),
			'total_debit'		=> number_format($summary[1][0]['total_debit'], 2, '.', ','),
			'total_balance'		=> number_format($previous_balance - $summary[1][0]['total_debit']-$summary[0][0]['total_credit'], 2, '.', ','),
			'currency' 			=> $currency_details[0]['currency_icon'],
			'position' 			=> $currency_details[0]['currency_position'],
			);
...
public function supplier_balance_report($from_date=null,$to_date=null){
		...
		if(!empty($suppliers_transection_report)){
			foreach($suppliers_transection_report as $index=>$value){
				...
				{
					$suppliers_transection_report[$index]['balance']=$suppliers_transection_report[$index]['total_debit']-
					$suppliers_transection_report[$index]['total_credit']+
					$suppliers_transection_report[$index]['previous_balance'];
				}else{
...
// 		$suppliers_transection_report =$CI->Suppliers->supplier_balance_report($from_date,$to_date);
		...
		$supplier_tax_report =$CI->Suppliers->supplier_tax_report();
// var_dump($supplier_tax_report);
// die();
		$currency_details = $CI->Soft_settings->retrieve_currency_info();

		$data=array(
			'title'    =>display('supplier_tax_report'),
		'currency' =>$currency_details[0]['currency_icon'],

			'supplier_tax_report'=>$supplier_tax_report,
			);
		$singlesupplier = $CI->parser->parse('supplier/supplier_tax_report',$data,true);
