...
class Linvoice {
	...
	public function invoice_add_form()
	{
		...
		$zone_list = $CI->Customers->zone_list();
	...
	public function pos_invoice_add_form()
	{
		...
		$state_list = $CI->Customers->select_city_country_id($edit_data->country);
	...
	public function invoice_html_data($invoice_id)
	{
		...
		$shipping=$CI->db->select('*')->from('shipping_info')->where('order_id',$invoice_detail[0]['order_id'])->get()->result();
		...
		$created_at = (!empty($invoice_detail[0]['created_at'])) ? explode(' ', $invoice_detail[0]['created_at'])[1] : null;
		...
		$data=array(
			'title'			   =>$invoice_detail[0]['invoice'],
			'invoice_id'	   =>$invoice_detail[0]['invoice_id'],
			...
			'total_vat'        =>$invoice_detail[0]['total_vat'],
			'cgst'        =>$invoice_detail[0]['cgst'],
			'sgst'        =>$invoice_detail[0]['sgst'],
			'igst'        =>$invoice_detail[0]['igst'],

			'cr_no'	           =>$invoice_detail[0]['cr_no'],
			'customer_address' =>$invoice_detail[0]['customer_address_1'] ?? $invoice_detail[0]['customer_short_address'],
			...
			'shipping_cname'   =>isset($shipping[0]->customer_name) ? $shipping[0]->customer_name : $invoice_detail[0]['customer_name'],
			'shipping_cmobile' =>isset($shipping[0]->customer_mobile) ? $shipping[0]->customer_mobile : $invoice_detail[0]['customer_mobile'],
			'shipping_cemail'  =>isset($shipping[0]->customer_email) ? $shipping[0]->customer_email : $invoice_detail[0]['customer_email'],
			'shipping_address' =>$invoice_detail[0]['customer_address_1'] ?? $invoice_detail[0]['customer_short_address'],
			'paid_amount'	   =>$invoice_detail[0]['paid_amount'],
			'due_amount'	   =>$invoice_detail[0]['due_amount'],
			'invoice_details'  =>$invoice_detail[0]['invoice_details'],
			...
			'currency' 		   =>$currency_details[0]['currency_icon'],
			'position' 		   =>$currency_details[0]['currency_position'],
            'ship_customer_short_address'=>$invoice_detail[0]['customer_address_1'] ?? $invoice_detail[0]['customer_short_address'],
...
// 		die();
		...
		$data=array(
			...
			'customer_name'		  =>$invoice_detail[0]['customer_name'],
			'state'	      =>$invoice_detail[0]['state'],
			'ship_state'	      =>$invoice_detail[0]['ship_state'],
