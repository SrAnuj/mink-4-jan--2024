...
class Lproduct {
    ...
    public function product_add_form()
    {
        ...
        $CI->load->model('dashboard/Product_Location');
   	...
   	public function subscription_add_form()
	{
		$CI =& get_instance();
		$data = array(
				'title' 		=> display('new_order'),
			);
		$subscriptionForm = $CI->parser->parse('dashboard/subscription/subscription_form',$data,true);
	...
	{
		$CI =& get_instance();
		$subscription_list = $CI->Products->subscription_list();
// var_dump($subscription_list);
// die();
		if(!empty($subscription_list)){
			foreach($subscription_list as $k=>$v){
				$subscription_list[$k]['final_date'] = $CI->occational->dateConvert($subscription_list[$k]['date']);
			}
			$i=0;
			foreach($subscription_list as $k=>$v){$i++;
			   $subscription_list[$k]['sl']=$i;
			}
		}

		$currency_details = $CI->Soft_settings->retrieve_currency_info();
		$data = array(
				'title'    => display('manage_subscription'),
				'subscription_list' => $subscription_list,
				'currency' => $currency_details[0]['currency_icon'],
				'position' => $currency_details[0]['currency_position'],
			);
		$orderList = $CI->parser->parse('dashboard/subscription/subscription',$data,true);
		return $orderList;
	}
    ...
    public function product_list($filter,$links,$per_page,$page)
    {
        ...
        $products_list 	   = $CI->Products->product_list($filter);
		$number_of_products 	   = $CI->Products->product_list_count();
    ...
    public function product_edit_data($product_id)
    {
       ...
       $Product_Location  = $CI->Product_Location->retrieve_Location_editdata(urldecode($product_id));
       //print_r($Product_Location);die;
        ...
        $data=array(
            ...
            'retail_price' 	     => $product_detail[0]['retail_price'],
            'wholesale_price'    => $product_detail[0]['wholesale_price'],
            'sgst'    => $product_detail[0]['sgst'],
            'cgst'    => $product_detail[0]['cgst'],
			'igst'    => $product_detail[0]['igst'],

            'product_model' 	 => $product_detail[0]['product_model'],
           'hsn'    => $product_detail[0]['hsn'],

            'product_details' 	 => $product_detail[0]['product_details'],
            'unit' 				 => $product_detail[0]['unit'],
            'moq' 				 => $product_detail[0]['moq'],
            ...
            'is_perishable'     => $product_detail[0]['is_perishable'],
        );
       //print_r($data['product_location']);die;
