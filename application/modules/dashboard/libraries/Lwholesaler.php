...
class Lwholesaler {
	...
	//Supplier Add Form
	public function wholesaler_add_form()
	{ 
		$CI =& get_instance();
		$CI->load->model('dashboard/Wholesalers');
		$data = array(
				'title' => 'Add Wholesaler'
			);
			$WholesalerForm = $CI->parser->parse('wholesaler/add_wholesaler_form',$data,true);
	...
	public function wholesaler_list()
	{
		$CI =& get_instance();
		$CI->load->model('dashboard/Wholesalers');
		$wholesalers_list = $CI->Wholesalers->wholesaler_list();
		$i=0;
		if(!empty($wholesalers_list)){	
			foreach($wholesalers_list as $k=>$v){$i++;
			   $wholesalers_list[$k]['sl']=$i;
			}
		}
		$data = array(
				'title' => display('manage_wholesaler'),
				'wholesalers_list' => $wholesalers_list,
			);
		$wholesalerList = $CI->parser->parse('wholesaler/wholesaler',$data,true);
	...
	// }
	//Insert new supplier
	public function insert_wholesaler($data)
	{
		$CI =& get_instance();
		$CI->load->model('dashboard/Wholesalers');
        $result = $CI->Wholesalers->wholesaler_entry($data);
		if ($result == TRUE) {
			return TRUE;
		}else{
			return FALSE;
	...
	public function wholesaler_edit_data($wholesaler_id)
	{
		$CI =& get_instance();
		$CI->load->model('dashboard/Wholesalers');
		$wholesaler_detail = $CI->Wholesalers->retrieve_wholesaler_editdata($wholesaler_id);
		$data=array(
			'wholesaler_id' 	=> $wholesaler_detail[0]['wholesaler_id'],
			'bussiness_name' => $wholesaler_detail[0]['bussiness_name'],
			'address' 		=> $wholesaler_detail[0]['address'],
			'email' 		=> $wholesaler_detail[0]['email'],
			'password' 		=> $wholesaler_detail[0]['password'],
			'mobile' 		=> $wholesaler_detail[0]['mobile'],
			'city' 		=> $wholesaler_detail[0]['city'],
			'country' 		=> $wholesaler_detail[0]['country'],
			'website' 		=> $wholesaler_detail[0]['website'],
			'abn' 		=> $wholesaler_detail[0]['abn']
			);
			
		$chapterList = $CI->parser->parse('wholesaler/edit_wholesaler_form',$data,true);
		return $chapterList;
	}
