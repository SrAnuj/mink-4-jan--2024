...
class Lstore {
	...
	public function store_add_form()
	{
		...
		$data = array(
		 ...
		 $CI->db->select('id, product_name'); // Select only the id and name columns
		$CI->db->where('status', 1);
		$data['products'] = $CI->db->get('product_information')->result();
	...
	public function store_edit_data($store_id)
	{
		...
		$store_target_details = $CI->Store_targets_model->retrieve_store_editdata($store_id);	
		$data=array(
			...
			'target_value' => $store_target_details[0]['target_value'],
			'target_ramarks' => $store_target_details[0]['remarks'],

			);
		// $data=array(
		// 	'store_target' => $store_target_details
		// 	);
		 $CI->db->select('id, product_name'); // Select only the id and name columns
		$CI->db->where('status', 1);
		$data['products'] = $CI->db->get('product_information')->result();
		//$data['store_target'] = $store_target_details;
