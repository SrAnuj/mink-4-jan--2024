...
$("#updateCart").on("submit", function (e) {
  ...
  var discountType = $(this).find("select[name=discount_type]").val(); // Get the selected discount type
  var rowID = $(this).find("input[name=rowID]").val();

  // Determine the discount based on the selected discount type
  if (discountType === "fixed") {
    ...
    var percentageDiscount = (quantity * rate * discount) / 100;
    $("#discount_" + rowID).val(percentageDiscount);
    $("#total_discount_" + rowID).val(percentageDiscount);
    $("#all_discount_" + rowID).val(percentageDiscount);
  }
