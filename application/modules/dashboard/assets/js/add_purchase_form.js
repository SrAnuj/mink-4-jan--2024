...
function addPurchaseOrderField(divName) {
  if (count == limits) {
    ...
    newdiv.innerHTML =
      ...
      '" name="batch_no[' +
      count +
      ']" class="form-control text-right" placeholder="0"  /></td><td class="text-right"><input type="text" id="expiry_date_' +
      count +
      '" name="expiry_date[' +
      count +
      ...
      ']" id="price_item_' +
      count +
      '" class="price_item1 text-right form-control" readonly placeholder="0.00" min="0" onkeyup="calculate_add_purchase(' +
      count +
      ')" onchange="calculate_add_purchase(' +
      count +
      ...
      ');" id="discount_' +
      count +
      '"class="form-control text-right" placeholder="0.00" min="0"/></td></td><td><input type="number" name="vat[' +
      count +
      ']" id="item_vat_' +
      count +
      '"class="form-control text-right item_vat" placeholder="0.00" min="0" readonly />   </td><td><input type="number" name="cgst[' +
      count +
      ']" id="cgst_' +
      count +
      '"class="form-control text-right cgst" placeholder="0.00" min="0" readonly /></td><td><input type="number" name="igst[' +
      count +
      ']" id="igst_' +
      count +
      '"class="form-control text-right igst" placeholder="0.00" min="0" readonly /></td> <input class="total_tax text-right form-control" type="hidden" name="total_tax['+ count +']" id="total_tax_'+ count +'" placeholder="0.00"readonly="readonly" /><td class="text-right"><input class="total_price text-right form-control" type="text" name="total_price[' +
      count +
      ']" id="total_price_' +
      count +
...
//   alert(sl);
  ...
  var item_vat = $("#item_vat_" + sl).val();
    var cgst = $("#cgst_" + sl).val();
    var igst = $("#igst_" + sl).val();
    //  console.log('item_vat',item_vat);

    //  console.log('quantity',total_qntt);
 ...
 // console.log('price_item',price_item);
  $("#item_vat_" + sl).val(item_vat);
    ...
    $("#igst" + sl).val(igst);
  var total_vat;
  if (cgst !== undefined && cgst !== null && item_vat !== undefined && item_vat !== ''){
	    
	  var cgst_tax = (cgst*price_item*total_qntt)/100;
	  var sgst_tax = (item_vat*price_item*total_qntt)/100;
	   
	    total_vat = cgst_tax + sgst_tax;
	  ...
	  var igst_tax = (igst*price_item*total_qntt)/100;
	   total_vat = igst_tax;
	  $(".Product_Igst").show();
...
//console.log(cgst_tax,sgst_tax);
//var igst_tax = (igst*price_item*total_qntt)/100;
 //var total_vat = cgst_tax +  sgst_tax +  igst_tax;
 //console.log(total_vat);
// console.log('sgst',sgst_tax);
 //console.log('cgst',cgst_tax);
  var total_price = 
...
//     isNaN(this.value) ||
//       0 == this.value.length ||
//       (total_vat += parseFloat(cgst_tax));
//   });
//   $(".item_vat").each(function () {
//     isNaN(this.value) ||
//       0 == this.value.length ||
//       (total_vat += parseFloat(sgst_tax));
//   });
//     $(".igst").each(function () {
//     isNaN(this.value) ||
//       0 == this.value.length ||
//       (total_vat += parseFloat(igst_tax));
//   });
  ...
  $("#total_price_" + sl).val(total_price.toFixed(2));
    $("#total_tax_" + sl).val(total_vat.toFixed(2));
  ...
  $(".total_price").each(function () {
   ...
   $(".total_tax").each(function () {
    isNaN(this.value) ||
      0 == this.value.length ||
      (total_s_c += parseFloat(this.value));
  });
  
  
        //   total_vat +=cgst_tax +  sgst_tax +  igst_tax;
           var sub_t =total_vat;
           total_tax += sub_t;
  $("#total_purchase_vat").val(total_s_c.toFixed(2, 2));
  console.log('total_s_c',total_s_c);
      console.log('total_vat',total_vat);

//   console.log('total_price',total_price);

  $("#subTotal").val(gr_tot.toFixed(2, 2));
  $("#grandTotal").val((gr_tot + total_s_c).toFixed(2, 2));
}
