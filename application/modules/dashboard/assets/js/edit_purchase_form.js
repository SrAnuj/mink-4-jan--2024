...
function addPurchaseOrderField(divName){
    ...
    if (count == limits)  {
        ...
        newdiv.innerHTML ='<td><input type="text" name="product_name" required class="form-control product_name productSelection" onkeyup="product_pur_or_list('+count+');" placeholder="'+display('product_name')+'" id="product_name_'+count+'" tabindex="5" ><input type="hidden" class="autocomplete_hidden_value product_id_'+count+'" name="product_id[]" id="SchoolHiddenId"/><input type="hidden" class="sl" value="'+count+'"></td><td class="text-center"><div class="variant_id_div"><select name="variant_id[]" id="variant_id_'+count+'" class="form-control variant_id width_100p" required=""><option value=""></option></select></div><div><select name="color_variant[]" id="color_variant_'+count+'" class="form-control color_variant width_100p" ><option value=""></option></select></div></td><td class="text-right"><input type="text" id="batch_no_'+count+'" name="batch_no[]" value="'+batch_no+count+'" class="form-control text-right" placeholder="0" readonly /></td><td class="text-right"><input type="text" id="expiry_date_'+count+'" name="expiry_date[]" class="form-control datepicker" placeholder="'+display('enter_expire_date')+'"/></td><td class="text-right"><input type="number" id="avl_qntt_'+count+'" class="form-control text-right" placeholder="0" readonly /></td><td class="text-right"><input type="number" name="product_quantity[]" id="total_qntt_'+count+'" onkeyup="calculate_add_purchase('+count+')"  class="form-control text-right p_quantity" placeholder="0" min="0" required/></td><td><input type="number" name="product_rate[]"  id="price_item_'+count+'" class="price_item1 text-right form-control" placeholder="0.00" min="0"/></td><td><input type="number" name="discount[]" onkeyup="calculate_add_purchase('+count+');"onchange="calculate_add_purchase('+count+');" id="discount_'+count+'"class="form-control text-right" placeholder="0.00" min="0"/></td><td><input type="number" name="vat_rate[]" onkeyup="calculate_add_purchase('+count+');"onchange="calculate_add_purchase('+count+');" id="item_vat_rate_'+count+'"class="form-control text-right" placeholder="0.00" min="0"/></td><td><input type="text" name="cgst[]" onkeyup="calculate_add_purchase('+count+');"onchange="calculate_add_purchase('+count+');" id="cgst_'+count+'"class="form-control text-right cgst" placeholder="0.00" min="0" readonly/></td><td> <input type="text" name="igst[]" onkeyup="calculate_add_purchase('+count+');" onchange="calculate_add_purchase('+count+');" id="igst_'+count+'"class="form-control text-right igst" placeholder="0.00" min="0" readonly/></td><td><input type="number" name="vat[]" id="item_vat_'+count+'"class="form-control text-right item_vat" placeholder="0.00" min="0" readonly /></td><td class="text-right"><input class="total_price text-right form-control" type="text" name="total_price[]" id="total_price_'+count+'" placeholder="0.00" readonly="readonly" /> </td><td><button class="btn btn-danger text-right" type="button" value="'+display('delete')+'" onclick="deleteRow(this)">'+display('delete')+'</button></td>';
...
function calculate_add_purchase(sl) {
    ...
    var sgst=$("#item_vat_"+sl).val();
        var igst=$("#igst_"+sl).val();
    var cgst=$("#cgst_"+sl).val();


    var total_price  =total_qntt * price_item - ((total_qntt * price_item*item_discount)/100);
    var sgst_tax     =((price_item*sgst)/100);
        var cgst_tax     =((price_item*cgst)/100);
    var igst_tax     =((price_item*igst)/100);
console.log('sgst_tax', sgst_tax);
console.log('cgst_tax', cgst_tax);
console.log('igst_tax', igst_tax);
    var Total_Quantity=0;

    $("#item_vat_"+sl).val(sgst);
    $("#cgst" + sl).val(cgst);
    $("#igst" + sl).val(igst);
    $(".item_vat").each(function(){
        isNaN(this.value) || 0 == this.value.length || (total_vat += parseFloat(this.value))
    });
    $(".p_quantity").each(function(){
        isNaN(this.value) || 0 == this.value.length || (Total_Quantity += parseFloat(this.value))
    });
    ...
    $(".total_price").each(function() {
        isNaN(this.value) || 0 == this.value.length || (gr_tot += parseFloat(this.value))
               ...
               total_vat = (+cgst_tax +  +sgst_tax +  +igst_tax)*total_qntt;
...
function calculate_add_purchase_cost(c){
    ...
    $(".purchase_expences").each(function(){
        isNaN(this.value) || 0 == this.value.length || (total_cost += parseFloat(this.value))
    });
