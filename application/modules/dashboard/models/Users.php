...
class Users extends CI_Model
{
	...
	function check_valid_user($username, $password)
	{
      ...
      public function salesTarget($id = null)
      {
          $sales_target = $this->db->select('*')->from('sec_user_access_tbl')->where('fk_user_id',  $id)->get()->result();
          return $sales_target;
      }
	  public function getCustomerSubscriptionData(){
	$this->db->select('subscription.*, product_information.*,customer_information.*');
    $this->db->from('subscription');
    $this->db->join('product_information', 'subscription.product_id = product_information.product_id');
	$this->db->join('customer_information', 'subscription.customer_id = customer_information.customer_id');
...
public function countCustomerSubscription() {
    $this->db->select('subscription.*, product_information.*, customer_information.*');
    $this->db->from('subscription');
    $this->db->join('product_information', 'subscription.product_id = product_information.product_id');
    $this->db->join('customer_information', 'subscription.customer_id = customer_information.customer_id');
    ...
    $expiredCount = 0; // Initialize the count of expired subscriptions.
