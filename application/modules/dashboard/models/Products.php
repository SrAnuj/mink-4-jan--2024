...
// 		$customer_id = $this->session->userdata('customer_id');
		$this->db->select('a.*,b.customer_name,c.product_name,c.product_model,d.batch_no');
		$this->db->from('subscription a');
		$this->db->join('customer_information b','b.customer_id = a.customer_id');
		$this->db->join('product_information c','c.product_id = a.product_id');
		$this->db->join('transfer d','d.product_id = c.product_id');
    ...
    public function product_list($filter = null,$per_page = null, $page = null)
    {
        ...
        // product_information.cgst,
        // product_information.sgst,
        $this->db->select('
            ...
            product_information.moq,
            product_information.price,
            product_information.supplier_price,
            product_information.retail_price,
            product_information.wholesale_price,
            product_information.onsale_price,

            product_information.image_thumb,
			product_information.is_perishable,
        ...
        $this->db->group_by('product_information.product_id');
        // check by echoing sql query here
        // echo $this->db->getCompiledSelect();
        // return;
        $query = $this->db->get();
    ...
    //         $this->db->delete('product_information');
	   //         return true;
        ...
        if ($affected_row == 0) {
            ...
            $product_info = $this->db->select('image_large_details, image_thumb')->from('product_information')->where_in('product_id',$ids)->get()->result();
            if($product_info){
                @unlink(FCPATH.$product_info->image_large_details);
                @unlink(FCPATH.$product_info->image_thumb);
            }
            //product gallery image delete
            $gallery_images = $this->db->select('image_url')->from('image_gallery')->where_in('product_id',$ids)->get()->result();
            if($gallery_images) {
            ...
            // //delete filter
            // $this->db->delete('filter_product', array('product_id' => $ids));
            // //delete filter
            // $this->db->where_in('product_id', $ids);
...
// multiple products delete
    ...
    {
        $this->db->select('a.*,b.*,c.supplier_name, d.variant_name,e.quantity');
        $this->db->from('product_purchase a');
        $this->db->join('product_purchase_details b', 'b.purchase_id = a.purchase_id','left');
		$this->db->join('purchase_stock_tbl e', 'e.product_id = b.product_id','left');
		...
		$this->db->group_by('b.product_id'); // Group by the product_id
