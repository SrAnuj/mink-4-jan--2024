...
class Wholesalers extends CI_Model {
	public function __construct()
	{
		parent::__construct();
	}
	//Count supplier
	public function count_wholesaler()
	{
		return $this->db->count_all("wholesaler_information");
	}
	//supplier List 
	public function wholesaler_list()
	{
		$this->db->select('*');
		$this->db->from('wholesaler_information');
		$this->db->order_by('bussiness_name','asc');
		$query = $this->db->get();
		if ($query->num_rows() > 0) {
	...
	//Count supplier
	public function wholesaler_entry($data)
	{
		// var_dump($data);
		// die(); 
		$this->db->select('*');
		$this->db->from('wholesaler_information');
		$this->db->where('bussiness_name',$data['bussiness_name']);
		$query = $this->db->get();
		...
		if ($query->num_rows() > 0) {
			...
			$result = $this->db->insert('wholesaler_information',$data);
// 			var_dump($result);
// 		die(); 
...
// 					$this->account_model->insert_supplier_head($data,$wholesaler_id);
...
// 			file_put_contents($cache_file,$productList);
	...
	public function retrieve_wholesaler_editdata($wholesaler_id)
	{
		$this->db->select('*');
		$this->db->from('wholesaler_information');
		$this->db->where('wholesaler_id',$wholesaler_id);
		$query = $this->db->get();
	...
	//Update Categories
	public function update_wholesaler($data,$wholesaler_id){
		$this->db->where('wholesaler_id',$wholesaler_id);
		$result = $this->db->update('wholesaler_information',$data); 	

		// Insert chart of account data
		if($result){
			$this->load->model('accounting/account_model');
			$this->account_model->update_supplier_head($data, $wholesaler_id);
		}
		...
		$query = $this->db->get();
		foreach ($query->result() as $row) {
			$json_product[] = array('label'=>$row->bussiness_name,'value'=>$row->wholesaler_id);
		}
		$cache_file = './my-assets/js/admin_js/json/supplier.json';
		$productList = json_encode($json_product);
		file_put_contents($cache_file,$productList);
	...
	public function delete_wholesaler($wholesaler_id)
	{
		...
		if ($result > 0) {
		    	...
		    	$this->db->where('wholesaler_id',$wholesaler_id);
			$result = $this->db->delete('wholesaler_information');
						return true;
			...
			redirect('dashboard/Cwholesaler/manage_wholesaler');
