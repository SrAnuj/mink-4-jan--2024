...
class Customer_dashboards extends CI_Model {
	...
	public function get_ordered_assign_delivery_boy($order_id)
    {
       $this->db->select('delivery_boy.*');
		$this->db->from('delivery_boy');
		$this->db->join('delivery_assign', 'delivery_boy.id = delivery_assign.delivery_boy_id', 'inner');
		$this->db->join('delivery_orders', 'delivery_orders.delivery_id = delivery_assign.delivery_id', 'inner');
		$this->db->where('delivery_orders.order_no', $order_id);
		$query = $this->db->get();
