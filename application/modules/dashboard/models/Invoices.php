...
class Invoices extends CI_Model
{
	...
	public function total_store_invoice()
	{
		...
		$this->db->where('store_id', $this->session->userdata('store_id'));
        $this->db->update('store_targets', ['target_achieved' => $count,'status' => 1]); */
	...
	public function getStoreReport(){
		...
		$this->db->where('st.store_id', $this->session->userdata('store_id'));// Filter for completed targets
	...
	{
		$this->db->select('a.*, b.*, c.order, b.customer_name');
		$this->db->from('invoice a');
		$this->db->join('customer_information b', 'b.customer_id = a.customer_id', 'inner');
		$this->db->join('order c', 'c.order_id = a.order_id', 'left');
		if (!empty($filter['invoice_no'] != '')) {
			$this->db->where('a.invoice', $filter['invoice_no']);
		}
	...
	public function invoice_entry()
	{
		...
		if (check_module_status('accounting') == 1) {
			...
			if (!empty($find_active_fiscal_year)) {
				...
				if ($this->input->post('customer_id', TRUE)) {
					...
					$data = array(
						...
						'previous_balance'      => (int) $this->input->post('previous_balance', TRUE),
				...
				$data = array(
					...
					'token'		  => 'tkn-' . $this->number_generator(),
				...
				$total_discount = $this->input->post('total_discount', TRUE);
				$total_price_before_discount = ($total_with_vat - $tota_vat) + (float) $total_discount;
				$store_id  = $this->input->post('store_id', TRUE);
			...
			for ($i = 0, $n = count($quantity); $i < $n; $i++) {
				...
				$variant_color    = $color_variants[$i];
				$batch            = $batch_no[$i];
				 $expiry_date      = date('d-m-Y', strtotime($expiry_date[$i]));
				$supplier_rate	  = $this->supplier_rate($product_id);
				$cogs_price      += ($supplier_rate[0]['supplier_price'] * $product_quantity);
		...
		if (check_module_status('accounting') == 1) {
			...
			if (!empty($find_active_fiscal_year)) {
				...
				$data = array(
					...
					'invoice'         => 'Inv-' . $this->number_generator(),
					'token'           => 'tkn-' . $this->number_generator(),
					'total_discount'  => null !== $this->input->post('total_discount', TRUE) ? $this->input->post('total_discount', TRUE) : null,


					'total_vat'       => $tota_vati, // Make sure $tota_vati is defined and contains the correct value
					'invoice_discount' => $invoice_data['invoice_discount'],
				...
				//d($result);die;
				/* $this->db->select('*');
				$this->db->where('invoice_id', $lastInsertedId);
				...
				$result = $query->row(); */
					...
					$this->db->select('date,store_id,SUM(total_amount) AS total_amount');
					...
					// Update the store_targets table
					/* $this->db->select('occupied_targets');
					$this->db->where('target_date', $result->date);
					$row001 = $this->db->get('daily_target_achievements');
					$currentTargetValue = $row001->occupied_targets;
					// Calculate the new occupied_targets by adding the invoice total amount
					$newTargetValue = $currentTargetValue + $totalAmount;
					 */
					$this->db->where('target_date', $result->date);
					$this->db->where('store_id', $result->store_id);

					// Use the += operator to increment the existing value of occupied_targets
					$this->db->set('occupied_targets', 'occupied_targets + ' . $totalAmount, FALSE);

					$row_affected = $this->db->update('daily_target_achievements');


					if($row_affected){
						// Get the updated daily_target_achievements for the specific store_id
						$this->db->select('store_id, SUM(occupied_targets) AS occupied_targets');
						$this->db->where('store_id', $result->store_id);
						$updatedTargets = $this->db->get('daily_target_achievements')->row();

						$this->db->where('store_id', $result->store_id);
						$this->db->set('target_achieved', $updatedTargets->occupied_targets);
						$this->db->update('store_targets');
				...
				for ($i = 0, $n = count($quantity); $i < $n; $i++) {
					...
					$total_price      = $total_amount[$i];
					$variant_id       = $variants[$i];
					$variant_color    = $color_variants[$i]?? 0;
					$batch            = $batch_no[$i];
					$supplier_rate	  = $this->supplier_rate($product_id);
					$cogs_price      += ($supplier_rate[0]['supplier_price'] * $product_quantity);
				...
				$cogs_price = $cogs_price;
				$total_discount = $this->input->post('total_discount', TRUE);
				$total_with_vat = floatval($total_with_vat); // Convert to float
				$tota_vat = floatval($tota_vat); // Convert to float
				$total_discount = floatval($total_discount); // Convert to float
...
// 		die();
	...
	public function get_total_product($product_id)
	{
		...
		$this->db->select('product_information.*,product_purchase_details.expiry_date');
$this->db->from('product_information');
$this->db->join('unit', 'unit.unit_id = product_information.unit', 'left');
$this->db->join('product_purchase_details', 'product_purchase_details.product_id = product_information.product_id', 'left');
...
$product_information = $this->db->get()->row();
		...
		if ($user_type == 6) {
			...
			$amount = ($product_information->retail_price * 1);
		}
		if ($user_type == 7) {
			$amount = ($product_information->wholesale_price * 1);
		}
		...
		if ($user_type == 6) {
			...
			$product_information->price = ($product_information->retail_price * floatval($product_information->moq));
		}
		if ($user_type == 7) {
			$product_information->price = ($product_information->wholesale_price * floatval($product_information->moq));
		}
		//echo $amount.' '.$product_information->price;die;
		$html = $colorhtml = "";
...
// 		New tax calculation for discount
		...
		//echo $amount.' '.$product_information->price;die;
		  ...
		  $base_price = $mrp / (1 + ($cgst / 100) + ($sgst / 100));
		$base_price  = number_format($base_price, 2, '.', '');
		//echo $amount.' '.$base_price;die;
		$data2 = array(
		'mrp'	 => @$mrp,
			'total_product'	 => @$stock,
			'supplier_price' => @$product_information->supplier_price,
			...
			'supplier_id' 	 => @$product_information->supplier_id,
			'product_name' 	 => @$product_information->product_name,
			'expiry_date' 	 => @$product_information->expiry_date,
			'product_model'  => @$product_information->product_model,
			'product_id' 	 => @$product_information->product_id,
			'variant' 		 => @$html,
			'colorhtml' 	 => @$colorhtml,
...
// 			'igst_id' 		 => (!empty($tax['igst_id']) ? $tax['igst_id'] : null),
	...
	{
		$query = $this->db->select('
					a.product_id,a.product_name,a.price,a.image_thumb,a.variants,a.product_model,a.sgst,a.cgst,
	...
	public function get_product_batches($product_id, $store_id=null)
	{
		$this->db->select('batch_no');
		$this->db->from('transfer');
		$this->db->where('product_id', $product_id);

		$query_transfer = $this->db->get();
		$transfer_batch = $query_transfer->result();
		$this->db->reset_query();
		...
		$merged_batches = [];
		foreach ($transfer_batch as $batch) {
		    $merged_batches[] = $batch->batch_no;
		}

		foreach ($batches as $batch) {
		    $merged_batches[] = $batch->batch_no;
		}
		if (!empty($merged_batches)) {
			$html = '<option value=""></option>';
			foreach ($merged_batches as $batch) {
