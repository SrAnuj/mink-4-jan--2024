...
class Retailers extends CI_Model {
	public function __construct()
	{
		parent::__construct();
	}
	//Count supplier
	public function count_retailer()
	{
		return $this->db->count_all("retailer_information");
	}
	//supplier List 
	public function retailer_list()
	{
		$this->db->select('*');
		$this->db->from('retailer_information');
	...
	//Count supplier
	public function retailer_entry($data)
	{
		...
		if ($query->num_rows() > 0) {
			...
			$result = $this->db->insert('retailer_information',$data);
			
			$retailer_id = $data['company_name'];
		// 	var_dump($retailer_id);
		// die();
			// Insert chart of account data
			if($result){
				if(check_module_status('accounting') == 1){
					$this->load->model('accounting/account_model');
					$this->account_model->insert_supplier_head($data,$retailer_id);
	...
	public function retrieve_supplier_editdata($retailer_id)
	{
		$this->db->select('*');
		$this->db->from('retailer_information');
	...
	//Update Categories
	public function update_retailer($data,$retailer_id)
	{
		$this->db->where('retailer_id',$retailer_id);
		$result = $this->db->update('retailer_information',$data); 	
	...
	public function delete_retailer($retailer_id)
	{
		...
		if ($result > 0) {
			$this->db->where('retailer_id',$retailer_id);
			$result = $this->db->delete('retailer_information');
						return true;
...
// 			file_put_contents($cache_file,$productList);
			...
			redirect('dashboard/Cretailer/manage_retailer');
		}
	}
