    ...
    die(); */
		...
		foreach($available_quantity as $k => $v)
		{
		    if($v < $quantity[$k])
		    {
		       ...
		       redirect('dashboard/Cproduct/manage_subscription');
		    }
		}
...
// 		die();
	...
	public function countUserOrder(){
		
		$this->db->select('a.*,d.*,  CONCAT(d.first_name, " ", d.last_name) as customer_name, IFNULL(c.invoice_status, 0) as invoice_status');
		$this->db->from('order a');
		$this->db->join('users d', 'd.user_id = a.user_id', 'left');
		$this->db->join('invoice c', 'c.order_id = a.order_id', 'left');
		if (!empty($this->session->userdata())) {
				$user_type_data = $this->session->userdata();
    ...
    public function order_list($filter = [], $page, $per_page)
    {
		 ...
		 $u_session = $this->session->userdata();
		if ($u_session['user_type'] == 6 || $u_session['user_type'] == 7) {
				$this->db->select('a.*,d.*,  CONCAT(d.first_name, " ", d.last_name) as customer_name, IFNULL(c.invoice_status, 0) as invoice_status');
				$this->db->from('order a');
		}else{
		$this->db->select('a.*,b.customer_name,z.delivery_zone, IFNULL(c.invoice_status, 0) as invoice_status');
				$this->db->from('order a');
		...
		$this->db->join('invoice c', 'c.order_id = a.order_id', 'left');
		$this->db->join('delivery_zone z', 'z.id = b.customer_zone', 'left');
		if (!empty($filter['order_no'])) {
			$this->db->where('a.order', $filter['order_no']);
		}
		...
		if (!empty($filter['customer_name'])) {
			if ($u_session['user_type'] == 6 || $u_session['user_type'] == 7) {
				...
				$this->db->where("CONCAT(d.first_name, ' ', d.last_name) LIKE '%" . $this->db->escape_like_str($filter['customer_name']) . "%' ESCAPE '!'", NULL, FALSE);
	...
	public function order_remarks_list($invoice_id)
    {
       $this->db->select('*');
		$this->db->from('delivery_assign');
		$this->db->join('delivery_orders', 'delivery_assign.delivery_id = delivery_orders.delivery_id', 'inner');
		$this->db->where('delivery_orders.order_no', $invoice_id);
		$query = $this->db->get();
		$result = $query->result_array(); // Fetch the result as an array of objects
		return $result;
	...
	public function retrieve_subscription_editdata($order_id)
    {
				$this->db->select('
			a.*,
			b.customer_name,
		...
		$this->db->from('order a');
		$this->db->join('customer_information b', 'b.customer_id = a.customer_id');
		$this->db->join('subscription s', 's.order_no = a.order');  // Join with the subscription table
		$this->db->join('product_information d', 'd.product_id = s.product_id');  // Join with the product_information table using the subscription's product_id
    ...
    public function get_total_product($product_id)
    {
		...
		if ($user_type == 6) {
			$product_information->price = $product_information->retail_price;
		} elseif ($user_type == 7) {
			$product_information->price = $product_information->wholesale_price;
		}
