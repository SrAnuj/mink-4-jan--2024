...
class Suppliers extends CI_Model {
	...
	public function isSupplierExist($mobileNumber, $bankAccountNumber) {
        $this->db->select('*');
        $this->db->from('supplier_information');
        $this->db->where('mobile_number', $mobileNumber);
        $this->db->or_where('bank_account_number', $bankAccountNumber);
        $query = $this->db->get();
	...
	public function supplier_tax_report()
	{
     	...
     	$this->db->select('c.supplier_name,c.mobile,c.vat_no,c.cin ,SUM((d.cgst*d.supplier_price)/100) as total_cgst,SUM((d.sgst*d.supplier_price)/100) as total_sgst,SUM((d.igst*d.supplier_price)/100) as total_igst ');
		$this->db->from('product_information d');
				$this->db->where('d.supplier_id IS NOT NULL');
		$this->db->join('supplier_information c','c.supplier_id = d.supplier_id','left');

	$this->db->group_by('c.supplier_name');
// 		var_dump($result);
// 		die();
 ...
 public function supplier_balance_report($from_date=null,$to_date=null){
      	$this->db->select('a.HeadCode,SUM(b.Debit) as total_debit,SUM(b.Credit) as total_credit,c.supplier_name,c.mobile,c.vat_no,c.cin,c.previous_balance');
