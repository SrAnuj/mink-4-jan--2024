...
class Delivery_system extends CI_Model {
	...
	public function retrieve_exists_delivery_order()
    {
		
		$this->db->from('subscription a');
		$this->db->join('delivery_orders b', 'a.order_no = b.order_no');
		//$this->db->join('order c', 'c.order = b.order_no');
		$query = $this->db->get();
    ...
    public function get_assigned_deliveries($per_page = null, $page = null,$delivery_boy_id = null, $start_date = null, $end_date = null, $delivery_zone = null){
		...
		if ($start_date !== null) {
            $this->db->where("STR_TO_DATE(a.completed_at, '%Y-%m-%dT%H:%i') >=", $start_date);
        }

        if ($end_date !== null) {
           $this->db->where("STR_TO_DATE(a.completed_at, '%Y-%m-%dT%H:%i') <=", $end_date);
        }

        // Check if $delivery_zone is not NULL before adding it to the query
        if ($delivery_zone !== null) {
            $this->db->where('c.delivery_zone', $delivery_zone);
        }

		if ($delivery_zone !== null) {
           $this->db->where('a.delivery_boy_id', $delivery_boy_id);
        }
        $this->db->limit($per_page, $page);
        $this->db->order_by('a.delivery_id','desc');
        $result = $this->db->get()->result_array();
    ...
    public function assigned_delivery_delete($delivery_id){
	 ...
	 public function get_filtered_deliveries($delivery_boy_id, $start_date, $end_date, $delivery_zone) {
        $this->db->select('a.*, b.name, c.delivery_zone, d.title');
        $this->db->from('delivery_assign a');
        $this->db->join('delivery_boy b', 'a.delivery_boy_id = b.id');
        $this->db->join('delivery_zone c', 'a.delivery_zone_id = c.id');
