...
class Admin_dashboard extends MX_Controller
{
    ...
    public function index()
    {
		...
		$user_order_count   = $this->Orders->countUserOrder();
		...
		$get_all_target = $this->Invoices->getStoreReport();
		$all_stores = $this->Store_targets_model->getAllStores();
		$all_store_target = $this->Store_targets_model->getStoreTargetReport();
		//d($all_stores);
    	//die;
        ...
        $data = array(
			...
			'all_store_target' => $all_store_target,
...
//check user exists on the database or not
    ...
    public function getUserRoleTarget(){
             $this->load->model('dashboard/UserTarget');
        $sales_manager_target = $this->UserTarget->getSalesManagers();

        $data['sales_manager_target'] = $sales_manager_target; // Pass the notifications to the view
        //print_r($sales_manager_target);die;
       $getSalesManagers = $this->UserTarget->getSalesExecutive();
       
        $data['sales_executive'] = $getSalesManagers;
        
       // $this->load->view('notifications/all_notification', $data);
         $content = $this->parser->parse('users_targets/index',$data,true);
        $this->template_lib->full_admin_html_view($content);
    }
}