...
class Cbarcode extends MX_Controller
{
    ...
    public function barcode_print($product_id)
    {
		...
		$this->db->select('product_category.category_name');
			$this->db->from('product_category');
			$this->db->where('category_id', $product_info[0]['category_id']);
			$query = $this->db->get();
			$row = $query->result_array();
		$qr_data = 'Name : '.$product_info[0]['product_name'] . "\n" .
					'Category : '.$row[0]['category_name'] . "\n" .
				   'Model : '.$product_info[0]['product_model'] . "\n" .
				   'Price : '.$product_info[0]['price']." INR".  "\n" .
				   'Short Description : '.$product_info[0]['tag'] . "\n" .
				   'Warrantee : '.$product_info[0]['warrantee'] . "\n" .
				   'Company Name : '.$company_info[0]['company_name']. "\n" .
				   'for more info : '.base_url() . 'product/'.remove_space($product_info[0]['product_name']).'/'.urlencode($product_id);
				//$params['data'] = $qr_data; 
				$params = array(
					'data' => $qr_data,
					'filepath' => 'path_to_save_qr_code.png',
					'code_type' => 'code39'
				);

				//$this->barcode_generator($product_id,$params);
        $data = array(
    ...
    public function barcode_create($filepath = "", $text = "0", $size = "20", $orientation = "horizontal", $code_type = "code128", $print = false, $SizeFactor = 1)
    {
        ...
        if ($filepath == "") {
            ...
            imagedestroy($image);
				...
				if (is_array($filepath) && array_key_exists(0, $filepath)) {
					
					$directory = dirname($filepath[0]);

					if (!is_dir($directory)) {
						mkdir($directory, 0755, true);
					}

					$filepath = $filepath[0]; // Get the actual file path from the array
...
// Now $filepath should contain a valid file path
