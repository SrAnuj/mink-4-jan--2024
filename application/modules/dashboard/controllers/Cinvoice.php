...
class Cinvoice extends MX_Controller
{
    ...
    public function new_invoice()
    {
        ...
        if (check_module_status('accounting') == 1) {
            ...
            if (!empty($find_active_fiscal_year)) {
				...
				$zone_list 	= $this->Customers->zone_list();
                $data = array(
    ...
    public function manage_invoice()
    {
        ...
        if (!empty($invoices_list)) {
            foreach ($invoices_list as $k => $v) {
				...
				//$this->occational->dateConvert($invoices_list[$k]['date']);
            }
            $i = 0;
            foreach ($invoices_list as $k => $v) {
                $i++;
    ...
    public function insert_pos_invoice()
    {
        ...
        if ($stok_report > 0) {
            ...
            // die();
			$product_details['expiry_date'] = date('d-m-Y',strtotime($product_details['expiry_date']));
            $html = "";
            ...
            if (!empty($product_details)) {
				...
				<th id=\"expiry_date_" . $product_id . "\"><input type=\"text\" name=\"expiry_date[]\" class=\"form-control productSelection \" value=\"" . $product_details['expiry_date'] . "\" placeholder=\"Expiry Date\" required readonly> </th>
				<td>
				<script>
                ...
                // if ($tax['cgst_status'] == 1) {
                    ...
                    $tr .= "<input type=\"hidden\" id=\"cgst_" . $product_id . "\" class=\"cgst\" value='" . $product_details['cgst_tax'] . "'/>
						<input type=\"hidden\" id=\"total_cgst_" . $product_id . "\" class=\"total_cgst\" name=\"cgst[]\" value='" . $product_details['cgst'] . "'/>
						<input type=\"hidden\" name=\"cgst_id[]\" id=\"cgst_id_" . $product_id . "\" value='" . $product_details['cgst_tax'] . "'/>";
                // }
                // if ($tax['sgst_status'] == 1) {

                    $tr .= "<input type=\"hidden\" id=\"sgst_" . $product_id . "\" class=\"sgst\" value='" . $product_details['sgst_tax'] . "'/>
						<input type=\"hidden\" id=\"total_sgst_" . $product_id . "\" class=\"total_sgst\" name=\"sgst[]\" value='" . $product_details['sgst'] . "'/>
						<input type=\"hidden\" name=\"sgst_id[]\" id=\"sgst_id_" . $product_id . "\" value='" . $product_details['sgst_tax'] . "'/>";
                // }
                // if ($tax['igst_status'] == 1) {
                    ...
                    $tr .= "<input type=\"hidden\" id=\"igst_" . $product_id . "\" class=\"igst\" value='" . $product_details['igst_tax'] . "'/>
						...
						<input type=\"hidden\" name=\"igst_id[]\" id=\"igst_id_" . $product_id . "\" value='" . $product_details['igst_tax'] . "'/>";
                // }
        ...
        foreach ($products as $product) {
            $html .= '<div class="col-xs-6 col-sm-4 col-md-2 col-p-3">
                    <div class="panel panel-bd product-panel select_product">
                        ...
                        <div class="panel-footer">' . $product->product_name . '-(' . $product->cgst . ')' . '</div>
                    </div>
                </div>';
        }
