...
class Cdelivery_system extends MX_Controller
{
    ...
    public function manage_assigned_delivery()
    {
		...
		$date_ranger = $this->input->get('date_ranger') ?? null;
		$start_date = null;
		$end_date = null;
		if ($date_ranger !== null) {
			list($start_date, $end_date) = explode(' to ', $date_ranger);
		}
		$delivery_boy_id = $this->input->get('delivery_boy_id') ?? null;
		$delivery_zone = $this->input->get('delivery_zone_id') ?? null;
        $assigned_deliveries = $this->Delivery_system->get_assigned_deliveries($config["per_page"], $page,$delivery_boy_id, $start_date, $end_date, $delivery_zone);
		 $delivery_boys  = $this->Delivery_system->get_active_delivery_boy();
        $delivery_zones = $this->Delivery_system->get_active_delivery_zone();
        $data = array(
