...
class Cproduct extends MX_Controller
{
    ...
    {
        $content = $this->lproduct->subscription_list();
        $this->template_lib->full_admin_html_view($content);
    }
    ...
    {     $data = array(
        ...
        $content = $this->lproduct->subscription_add_form();
        $this->template_lib->full_admin_html_view($content);
    }
	public function insert_subscription()
    {
        ...
        // var_dump($result);
        // die();
        redirect(base_url('dashboard/Cproduct/manage_subscription'));
    }
    ...
    public function insert_product()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $is_perishable = $this->input->post('is_perishable', TRUE);
            // $retail_price = $price - $price*0.2;
            // $wholesale_price = $price - $price*0.3;
// $total_cgst = $cgst *$price / 100; 
// $total_sgst = $sgst *$price / 100; 
// $total_igst = $igst *$price / 100; 
// $total_price = $price + $total_cgst + $total_sgst;
            ...
            if ($igst) {
                $igst_tax = $this->input->post('igst_tax', TRUE);
                $igst_tax = (!empty($igst_tax) ? $igst_tax : null);
            } else {
                $igst_tax = null;
            }
           ...
           $city = $this->input->post('city', TRUE);
            ...
            if (!empty($city)){
                foreach ($city as $key => $value ) 
                {
                    ...
                    $t_data['city'] = $city[$key];
                  
                    $this->Product_Location->Product_location_entry($t_data);
                }
    ...
    public function product_update($product_id)
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $city = $this->input->post('city', TRUE);
            (int) $is_perishable = $this->input->post('is_perishable', TRUE);
            //var_dump((int) $is_perishable);die;
             if ($is_perishable == 0) {
             $this->Product_Location->delete_product_locations($product_id);
            }else{
                $this->Product_Location->delete_product_locations($product_id);
                if (!empty($city)){
                    foreach ($city as $key => $value ) 
    ...
    public function delete_products()
    {
        if (isset($_POST['ids'])) {
            ...
            $result = $this->Products->delete_products_by_ids($ids);
        ...
        if (($handle = fopen($_FILES['upload_csv_file']['tmp_name'], 'r')) !== FALSE) {
            ...
            while ($csv_line = fgetcsv($fp, 1024)) {
                ...
                for ($i = 0, $j = count($csv_line); $i < $j; $i++) {
                    ...
                    $insert_csv['retail_price'] = (!empty($csv_line[5]) ? $csv_line[5] : '');
                    $insert_csv['wholesale_price'] = (!empty($csv_line[6]) ? $csv_line[6] : '');

                    $insert_csv['cgst'] = (!empty($csv_line[7]) ? $csv_line[7] : '');
                    $insert_csv['sgst'] = (!empty($csv_line[8]) ? $csv_line[8] : '');
                    $insert_csv['hsn'] = (!empty($csv_line[9]) ? $csv_line[9] : '');
                    $insert_csv['igst'] = (!empty($csv_line[10]) ? $csv_line[10] : '');


                    $insert_csv['unit'] = (!empty($csv_line[11]) ? $csv_line[11] : '');
                    $insert_csv['moq'] = (!empty($csv_line[12]) ? $csv_line[12] : '');

                    $insert_csv['product_model'] = (!empty($csv_line[13]) ? $csv_line[13] : '');
                    $insert_csv['product_details'] = (!empty($csv_line[14]) ? $csv_line[14] : '');
                    $insert_csv['image_thumb'] = (!empty($csv_line[15]) ? $csv_line[15] : '');
                    $insert_csv['brand_id'] = (!empty($csv_line[16]) ? $csv_line[16] : '');
                    $insert_csv['variants'] = (!empty($csv_line[17]) ? $csv_line[17] : '');
                    $insert_csv['variant_prices'] = (!empty($csv_line[18]) ? $csv_line[18] : []);
                    $insert_csv['type'] = (!empty($csv_line[19]) ? $csv_line[19] : '');
                    $insert_csv['best_sale'] = (!empty($csv_line[20]) ? $csv_line[20] : 0);
                    $insert_csv['onsale'] = (!empty($csv_line[21]) ? $csv_line[21] : 0);
                    $insert_csv['onsale_price'] = (!empty($csv_line[22]) ? $csv_line[22] : '');
                    $insert_csv['invoice_details'] = (!empty($csv_line[23]) ? $csv_line[23] : '');
                    $insert_csv['image_large_details'] = (!empty($csv_line[24]) ? $csv_line[24] : '');
                    $insert_csv['review'] = (!empty($csv_line[25]) ? $csv_line[25] : '');
                    $insert_csv['description'] = (!empty($csv_line[26]) ? $csv_line[26] : '');
                    $insert_csv['tag'] = (!empty($csv_line[27]) ? $csv_line[27] : '');
                    $insert_csv['specification'] = (!empty($csv_line[28]) ? $csv_line[28] : '');
                    $insert_csv['status'] = (!empty($csv_line[29]) ? $csv_line[29] : 0);
                }
                ...
                $data = array(
                    ...
                    'retail_price' => $insert_csv['retail_price'],
                    'wholesale_price' => $insert_csv['wholesale_price'],
                    'cgst' => $insert_csv['cgst'],
                    'sgst' => $insert_csv['sgst'],
                    'hsn' => $insert_csv['hsn'],
                    'igst' => $insert_csv['igst'],
    ...
    public function product_excel_insert()
    {
        ...
        if ($datacount > 1) {
            for ($i = 1; $i < $datacount; $i++) {
                ...
                $retail_price = $sheetdata[$i][5];
                $wholesale_price = $sheetdata[$i][6];
                $cgst = $sheetdata[$i][7];
                $sgst = $sheetdata[$i][8];
                $hsn = $sheetdata[$i][9];
                $igst = $sheetdata[$i][10];
                ...
                $unit = $sheetdata[$i][11];
                $moq = $sheetdata[$i][12];
                $product_model = $sheetdata[$i][13];
                $product_details = $sheetdata[$i][14];
                $image_thumb = $sheetdata[$i][15];
                $brand_id = $sheetdata[$i][16];
                $variants = $sheetdata[$i][17];
                $variant_price = $sheetdata[$i][18];
                $type = $sheetdata[$i][19];
                $best_sale = $sheetdata[$i][20];
                $onsale = $sheetdata[$i][21];
                $onsale_price = $sheetdata[$i][22];
                $invoice_details = $sheetdata[$i][23];
                $image_large_details = $sheetdata[$i][24];
                $review = $sheetdata[$i][25];
                $description = $sheetdata[$i][26];
                $tag = $sheetdata[$i][27];
                $specification = $sheetdata[$i][28];
                $status = $sheetdata[$i][29];
                ...
                $product_details = array(
                    ...
                    'retail_price' => $excel['retail_price'],
                    'wholesale_price' => $excel['wholesale_price'],
                    'cgst' => $excel['cgst'],
                    'sgst' => $excel['sgst'],
                    'hsn' => $excel['hsn'],
                    'igst' => $excel['igst'],
                ...
                $result = $this->db->select('*')
                    ->from('product_information')
                    ->where('product_model', $product_details['product_model'])
                    ->get()
                    ->num_rows();
                // var_dump('other' ,$result );
                // die();
                if ($result == 0 && !empty($product_details['product_model']) && !empty($product_details['supplier_id'])) {
...
//                         die();
                    $this->db->where('supplier_id', $product_details['supplier_id']);
                    $this->db->where('product_model', $product_details['product_model']);
                    $this->db->update('product_information', $product_details);
                    ...
                    if (is_array($variant_prices)) {
                        ...
                        foreach ($variant_prices as $vitem) {
                            ...
                            if (is_array($vitem_list)) {
                                ...
                                $size_variant = trim($vitem_list[0]);
                                $color_variant = trim($vitem_list[1]);
                                //  var_dump($size_variant);
//  var_dump($color_variant);
                                // die();
                                if (empty($vitem_list[2])) {
                                    $color_variant = NULL;
                                    $variant_price_amt = trim($vitem_list[1]);
    ...
    public function delete_locations() {
        ...
        if ($this->input->is_ajax_request()) {
            // Get the attr_id from the AJAX request
            $location_id = $this->input->post('id');

            $deleted = $this->Product_Location->delete_locations($location_id);

            if ($deleted) {
                // If the record was successfully deleted, return a success response
                echo json_encode(['success' => true, 'message' => 'Product Location deleted successfully']);
            } else {
                // If there was an error deleting the record, return an error response
                echo json_encode(['success' => false, 'message' => 'Error deleting product location']);
            }
        } else {
            // If it's not an AJAX request, return an error response
