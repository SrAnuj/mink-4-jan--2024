        ...
        if (check_module_status('accounting') == 1) {
            ...
            if (!empty($find_active_fiscal_year)) {
                ...
                if ($this->form_validation->run() == TRUE) {
                    // echo $purchase_id;
                    // die();
                    $vat_rate = $this->db->select('total_purchase_tax')->from('product_purchase')->where('purchase_id', $purchase_id)->get()->row();
                    //   var_dump($vat_rate);
                    // die();
                    if (!empty($vat_rate)) {
                    ...
                    if ($result1) {
                        ...
                        foreach ($products as $key => $product) {
                            ...
                            if ($result2) {
                                ...
                                //   var_dump($product_purchase_details_info);
                            // die();
                                $product_cost = (($product_purchase_details_info->vat*$product_purchase_details_info->total_amount)/100  + $product_purchase_details_info->total_amount);
//  var_dump($product_cost);
                            // die();
                                $product_purchase_info = $this->db->select('total_items,sub_total_price,total_purchase_tax,grand_total_amount')->from('product_purchase')->where('purchase_id', $purchase_id)->get()->row();
                                ...
                                $new_product_purchase = array(
                                    ...
                                    'total_purchase_vat' => ($product_purchase_info->total_purchase_tax - $product_purchase_details_info->vat),
                                    'grand_total_amount' => ($product_purchase_info->grand_total_amount - $product_cost),
                                );
                            //   var_dump($new_product_purchase);
                            //   die();
