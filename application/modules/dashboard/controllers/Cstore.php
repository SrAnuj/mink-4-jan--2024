...
class Cstore extends MX_Controller
{
    ...
    function __construct()
    {
        ...
        //$this->load->model('dashboard/Store');
    ...
    public function insert_store()
    {
        ...
        if ($this->form_validation->run() === FALSE) {
			...
			$month = date('n'); // Current month (1 to 12)
			$year = date('Y');  // Current year
			$numberOfDays = cal_days_in_month(CAL_GREGORIAN, $month, $year);
			//echo "Number of days in the current month: $numberOfDays";die;
            $data = array(
           ...
           $remarks_value = $this->input->post('remarks_value', TRUE);
            ...
            $this->Store_targets_model->Store_target_entry($t_data);
           $monthlyTargetID = $this->db->insert_id();
		    //echo $monthlyTargetID;die;
			
			$this->db->select('*');
			$this->db->from('store_targets');
			$this->db->where('id', $monthlyTargetID);
			$target = $this->db->get()->row();
			// Get the result as an array
			
			
			$dailyTargetValue = round($target->target_value / 30);
			//print($dailyTargetValue);die;
			for ($day = 0; $day <= 30; $day++) {
				$targetDate = date('Y-m-d', strtotime("+$day days"));
				$month = date('m', strtotime($targetDate));

				// Adjust this CodeIgniter query based on your actual table structure
				$insertDailyTargetData = array(
				'store_id' => $data['store_id'],
					'store_target_id' => $monthlyTargetID,
					'target_month' => $month,
					'target_year' => $year,
					'target_date' => $targetDate,
					'daily_target_value' => $dailyTargetValue,
					'added_by' => $this->session->userdata['user_id'],
					'status' => 0,
					'added_date' => date('Y-m-d H:i:s')  // Current date and time
				);

				// Insert the data into the 'daily_target_achievements' table
				$this->db->insert('daily_target_achievements', $insertDailyTargetData);
    ...
    public function store_update($store_id = null)
    {
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            //$this->Store_targets_model->delete_store_target($store_id);
            //$product_ids = $this->input->post('product_id', TRUE);
           $target_value = $this->input->post('target_value', TRUE);
           $remarks_value = $this->input->post('remarks_value', TRUE);
        ...
        // Call the model method to update the target
        $updated_target_id = $this->Store_targets_model->update_target($store_id, $update_data);
            ...
            for ($day = 0; $day <= 30; $day++) {
				...
				// Extract the month from the target date
				$month = date('m', strtotime($targetDate));

				// Update the daily target value in the 'daily_target_achievements' table
				$this->db->set('daily_target_value', $dailyTargetValue);
				//$this->db->where('id', $target->id);
				$this->db->where('store_id', $target->store_id);
				$this->db->where('target_month', $month);
				$this->db->where('target_year', date('Y'));
				$this->db->where('target_date', $targetDate);
				$this->db->update('daily_target_achievements');
			}
			$this->db->select('*');
			$this->db->from('store_targets');
			$this->db->where('store_id', $store_id);
			$newTarget = $this->db->get()->row();
			if (!$newTarget) {
				$t_data = array(
                'store_id' =>$store_id,
                    'target_value' => $target_value,
    ...
    // Your controller method to delete a product attribute
    public function delete_store_target() {
        // Check if it's an AJAX request
        if ($this->input->is_ajax_request()) {
            // Get the attr_id from the AJAX request
            $attr_id = $this->input->post('id');

            $deleted = $this->Store_targets_model->delete_product_attribute($attr_id);

            if ($deleted) {
                // If the record was successfully deleted, return a success response
                echo json_encode(['success' => true, 'message' => 'Store target deleted successfully']);
            } else {
                // If there was an error deleting the record, return an error response
                echo json_encode(['success' => false, 'message' => 'Error deleting store target']);
            }
        } else {
