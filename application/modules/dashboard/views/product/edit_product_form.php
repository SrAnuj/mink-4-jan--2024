...
<div class="content-wrapper">
                        ...
                        <div id="rootwizard">
                            ...
                            <div class="tab-content">
                                <div class="tab-pane" id="tab1">
                                        ...
                                        <div class="col-sm-6">
                                            <div class="form-group row">
                                                <label for="category_id"
                                                    class="col-sm-3 col-form-label"><?php echo display('category') ?></label>
                                                <div class="col-sm-9">
                                                    <select class="form-control select2 width_100p" id="category_id"
                                                        name="category_id">
                                                        <?php foreach ($category_list as $category) { ?>
                                                        <option
                                                            value="<?php echo html_escape($category['category_id']) ?>"
                                                            <?php if ($category['category_id'] == $category_selected) {
                                                                                                                                    echo "selected";
                                                                                                                                } ?>>
                                    ...
                                    <div class="row">
                                        <div class="col-sm-12">
                                            <?php $fetched_filter_types = $this->db->select('a.*,b.fil_type_name')
                                        ...
                                        <div class="row text-center">
                                            <div class="col-sm-12">
                                              ...
                                              <label class="switch switch-left-right">
                                                ...
                                                (!empty($is_perishable == 1)) ? 'checked' : '' ?> id="toggleProductAttr" name="is_perishable">
                                                <span class="switch-label" data-on="perishable" data-off="None"></span> <span class="switch-handle"></span> </label>
                                                </div>
                                                
                                            </div>
                                            </div>
                                        </div> 
                                        <div class="row" id="add_product_available">
                                        <div class="col-sm-12">
                                           <div class="form-group row">
                                           <div class="col-sm-3 col-form-label">
                                               <a class="btn btn-primary" onclick="add_more_attr();">
                                                <i class="fa fa-plus" aria-hidden="true"></i> Add Product Availability
                                            </a>
                                           </div> 
                                            <div class="col-md-9" id="product_attr_box">
    <?php
    $attrProductloop = 1;
  $dynamicValues = $this->db->select('name')
                ->from('cities')
				->where('state_id',32)
                ->get()
                ->result_array(); 
    ?>
    <?php foreach($product_location as $key => $zipcode): ?>
        <div class="row" id="attr_<?php echo $attrProductloop; ?>">
            <div class="form-group col-md-6">
                <label class="control-label" for="product_id_<?php echo $attrProductloop; ?>">City <?php echo $attrProductloop; ?></label>
                <select class="form-control get_cities" id="product_id_<?php echo $attrProductloop; ?>" name="city[]">
                    <?php foreach ($dynamicValues as $dynamicValue): ?>
                        <option value="<?php echo $dynamicValue['name']; ?>" <?php echo ($dynamicValue['name'] === $zipcode['city']) ? 'selected' : ''; ?>><?php echo $dynamicValue['name']; ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div class="form-group col-md-3">
                <label class="control-label"></label>
                <a class="btn btn-danger btn-sm btn-block" style="width:25%" onclick="removeProductAttribute('<?php echo $attrProductloop; ?>','<?php echo $zipcode['id'] ?>')" role="button">
                    <i class="fa fa-trash" aria-hidden="true"></i>
                </a>
            </div>
        </div>
        <?php $attrProductloop++; endforeach; ?>
</div>
                                                ...
                                                <div class="col-sm-9">
                                                    <select class="form-control select2 width_100p" id="unit"
                                                        ...
                                                        <option value="<?php echo html_escape($unit['unit_id']) ?>"
                                                            <?php if ($unit['unit_id'] == $unit_selected) {
                                                                                                                            echo 'selected';
                                                                                                                        } ?>>
                                                ...
                                                <div class="col-sm-9">
                                                    <select class="form-control select2 width_100p" id="brand"
                                                        ...
                                                        <option value="<?php echo html_escape($brand['brand_id']); ?>"
                                                            <?php if ($brand['brand_id'] == $brand_selected) {
                                                                                                                                echo 'selected';
                                                                                                                            } ?>>
 ...
 <div class="form-group row">
                                        <label for="moq"
                                            class="col-sm-4 col-form-label">M.O.Q
                                            <span class="color-red">*</span></label>
                                        <div class="col-sm-4">
                                            <input type="number" tabindex="4" class="form-control text-right"
                                                name="moq" value="5" step="1"  min="5"
                                    ...
                                    <div class="form-group row">
                                        <label for="retail_price"
                                            class="col-sm-4 col-form-label">retail_price
                                            <span class="color-red">*</span></label>
                                        <div class="col-sm-4">
                                            <input type="number" tabindex="4" class="form-control text-right"
                                                name="retail_price" value="<?php echo html_escape($retail_price) ?>"
                                                placeholder="retail_price" required=""
                                                min="0" id="retail_price" />
                                        </div>
                                    </div>
                                     <div class="form-group row">
                                        <label for="wholesale_price"
                                            class="col-sm-4 col-form-label">wholesale_price
                                            <span class="color-red">*</span></label>
                                        <div class="col-sm-4">
                                            <input type="number" tabindex="4" class="form-control text-right"
                                                name="wholesale_price" value="<?php echo html_escape($wholesale_price) ?>"
                                                placeholder="wholesale_price" required=""
                                                min="0" id="wholesale_price" />
                                        </div>
                                    </div>
                                     <div class="form-group row">
                                        <label for="sell_price"
                                            class="col-sm-4 col-form-label"><?php echo 'SGST Tax';?> <span
                                                class="color-red">*</span></label>
                                        <div class="col-sm-4">
                                           <select class="select2 width_100p form-control" name="sgst" id="sgst">
                                        <option value="<?php echo html_escape($sgst) ?>" <?php echo "selected" ?>><?php echo html_escape($sgst) ?></option>
                                     ...
                                     <div class="form-group row">
                                        <label for="sell_price"
                                            class="col-sm-4 col-form-label"><?php echo 'CGST Tax'; ?> <span
                                                class="color-red">*</span></label>
                                        <div class="col-sm-4">
                                            <select class="select2 width_100p form-control" name="cgst" id="cgst">
                                        <option value="<?php echo html_escape($cgst) ?>" <?php echo "selected" ?>><?php echo html_escape($cgst) ?></option>
									...
									<div class="form-group row">
                                        <label for="onsale"
                                            class="col-sm-4 col-form-label"><?php echo 'IGST Tax Apply'; ?> <span
                                                class="color-red">*</span></label>
                                        <div class="col-md-4">
                                            <select class="form-control select2 width_100p" id="igst" name="igst"
                                                required="">
                                                <option value="0"><?php echo display('no') ?></option>
                                                <option value="1" <?php if(isset($igst) && $igst != ''){echo 'selected'; }else{ echo '';} ?>><?php echo display('yes') ?></option>
                                            </select>
                                        </div>
                                    </div>
                                    <div class="form-group row">
                                    ...
                                    <div class="form-group row onsale_price" style="<?php if ($onsale == 0) {
                                                                                        echo "display: none";
                                                                                    } ?>">
...
<div class="form-group row">
                                        <label for="model"
                                            class="col-sm-4 col-form-label"><?php echo 'H.S.N Code'; ?> <span
                                                class="color-red">*</span></label>
                                        <div class="col-sm-4">
                                            <input type="text" tabindex="5" class="form-control"
                                                value="<?php echo html_escape($hsn); ?>" name="hsn"
                                                placeholder="<?php echo 'H.S.N Code'; ?>" required />
                                        ...
                                        <div class="col-sm-4 custom_select">
                                            <select name="supplier_id" class="form-control select2" required=""
                                                ...
                                                <option value="<?php echo html_escape($supplier['supplier_id']) ?>"
                                                    <?php if ($supplier['supplier_id'] == $supplier_selected) {
                                                                                                                                echo 'selected';
                                                                                                                            } ?>>
                                        ...
                                        <div class="col-sm-4 custom_select">
                                            <select name="default_variant" class="form-control select2"
                                                ...
                                                <option value="<?php echo html_escape($v_item->variant_id)  ?>"
                                                    <?php if ($v_item->variant_id == $default_variant) {
                                                                                                                            echo "selected";
                                                                                                                        }  ?>>
                                                            ...
                                                            <tr id="row_<?php echo $i; ?>">
                                                                <td>
                                                                    <select name="size_variant[<?php echo $i; ?>]"
                                                                        ...
                                                                        <option
                                                                            value="<?php echo html_escape($v_item['variant_id']) ?>"
                                                                            <?php if (!empty($provar_prices[$i]->var_size_id) && ($provar_prices[$i]->var_size_id == $v_item['variant_id'])) {
                                                                                                                                                                    echo 'selected';
                                                                                                                                                                } ?>>
                                                                            <?php echo html_escape($v_item['variant_name']);  ?>
                                                                        </option>
                                                                ...
                                                                <td>
                                                                    <select name="color_variant[<?php echo $i; ?>]"
                                                                        ...
                                                                        <option
                                                                            value="<?php echo html_escape($v_item['variant_id']) ?>"
                                                                            <?php if (!empty($provar_prices[$i]->var_color_id) && ($provar_prices[$i]->var_color_id == $v_item['variant_id'])) {
                                                                                                                                                                    echo 'selected';
                                                                                                                                                                } ?>>
                                                                            <?php echo html_escape($v_item['variant_name']);  ?>
                                                                        </option>
                                                                ...
                                                                <td>
                                                                    <input type="text"
                                                                        ...
                                                                        class="form-control" placeholder="0.00"
                                                                        value="<?php if (isset($provar_prices[$i]->price) && !empty($provar_prices[$i]->price)) {
...
<script>
var dynamicValues = <?= json_encode($dynamicValues) ?>;
      if($('#toggleProductAttr').is(':checked')){
        $('#add_product_available').show();
        $('#toggleProductAttr').val(1);
    }else{
        $('#add_product_available').hide();
        $('#toggleProductAttr').val(0);
    }
  //$('#add_product_available').hide();

    // Handle the checkbox change event
    $('#toggleProductAttr').change(function() {
        if ($(this).is(':checked')) {
            // Checkbox is checked, show the add_product_available
            $('#add_product_available').show();
            $('#toggleProductAttr').val(1);
        } else {
            // Checkbox is not checked, hide the add_product_available
            $('#add_product_available').hide();
            $('#toggleProductAttr').val(0);
        }
    });
   
	var selectedCities = [];  // Array to store selected cities
...
var attr_count = 0;
    function add_more_attr(){
      ...
      var html = '<div class="row" id="attr_' + attr_count + '">' +
            '<div class="form-group col-md-6">' +
            '    <label class="control-label" for="product_id_' + attr_count + '">City ' + attr_count + '</label>' +
            '    <select class="form-control get_cities" id="product_id_' + attr_count + '" name="city[]">';

        // Adding options based on dynamicValues
        for (var i = 0; i < dynamicValues.length; i++) {
        if (selectedCities.indexOf(dynamicValues[i]['name']) === -1) {
            html += '<option value="' + dynamicValues[i]['name'] + '">' + dynamicValues[i]['name'] + '</option>';
        }
    }
        ...
        html += '    </select>' +
            ...
            '<div class="form-group col-md-3">' +
            '    <label class="control-label"></label>' +
            '    <a class="btn btn-danger btn-sm btn-block" style="width:25%" onclick=remove_attr("' + attr_count + '") role="button">' +
            '        <i class="fa fa-trash" aria-hidden="true"></i>' +
            '    </a>' +
            '</div>' +
            '</div>';
    ...
    if (selectedCities.indexOf(selectedCity) === -1) {
        // Add the selected city to the array
        selectedCities.push(selectedCity);
    } else {
        // Reset the selected value to prevent the same city from being selected
...
function removeProductAttribute(attr_count,id) {
    ...
    $.ajax({
        url: '<?= base_url('product/remove_location') ?>',
        type: 'POST',
 ...
 $(document).ready(function () {
              var updatingDropdown = false;

$("#sgst").change(function () {
    if (!updatingDropdown) {
        updatingDropdown = true;

        var selectedSgst = $(this).val();
        $("#cgst").val(selectedSgst);

        // Trigger the change event on cgst dropdown for Select2 to update its UI
        $("#cgst").trigger('change');
...
$("#cgst").change(function () {
    if (!updatingDropdown) {
        ...
        var selectedCGST = $(this).val();
        $("#sgst").val(selectedCGST);

        // Trigger the change event on sgst dropdown for Select2 to update its UI
        $("#sgst").trigger('change');
