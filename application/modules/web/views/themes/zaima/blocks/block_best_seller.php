    ...
    <div class="container">
        <div class="row">
			...
			$user_type = $this->session->userdata('user_type');
				...
				if ($user_type == 6) {
					if ($best_sales) {
						foreach ($best_sales as $key => $retail_product) {
							$best_sales[$key]->price = $retail_product->retail_price;
						}
					}
				}
				//7 is wholesaler 
				if ($user_type == 7) {
					if ($best_sales) {
						foreach ($best_sales as $key => $whole_product) {
							$best_sales[$key]->price = $whole_product->wholesale_price;
						}
					}
				}
			?>
