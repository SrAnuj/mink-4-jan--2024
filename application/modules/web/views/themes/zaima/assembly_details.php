                    ...
                    <div class="assembly_product_area">
                        ...
                        if(!empty($assembled_products)){
                         ...
                         foreach ($assembled_products as $assembled_product){
							...
							if ($user_type == 6) {
								if ($results) {
									foreach ($results as $key => $retail_product) {
										
										$results[$key]['price'] = $retail_product['retail_price'];
									}
								}
							}
							//7 is wholesaler 
							if ($user_type == 7) {
								if ($results) {
									foreach ($results as $key => $whole_product) {
										$results[$key]['price'] = $whole_product['wholesale_price'];
									}
								}
							}
                        ?>
