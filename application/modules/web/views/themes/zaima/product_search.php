    ...
    <div class="row">
        ...
        <div class="<?php echo (!empty($after_search) ? 'col-md-9' : 'col-md-9') ?> mainContent category-content">
            ...
            <div class="row">
				...
				if ($user_type == 6) {
					if ($category_product) {
						foreach ($category_product as $key => $product) {
							$category_product[$key]->price = $product->retail_price;
						}
					}
				}
				if ($user_type == 7) {
					if ($category_product) {
						foreach ($category_product as $key => $product) {
							$category_product[$key]->price = $product->wholesale_price;
						}
					}
				}
                    ...
                    <div class="feature-card card mb-3">
                        <div class="card-body">
                            <a href="<?php echo base_url('product/' . remove_space($product->product_name) . '/' . $product->product_id) ?>"
                                ...
                                <?php } else { ?>
                                <img class="img-fluid"
                                    src="<?php echo  base_url() .  $product->image_thumb ?>"
                                    alt="<?php echo html_escape($product->product_name) ?>">
                                <?php } ?>
                            </a>
