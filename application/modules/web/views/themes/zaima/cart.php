                ...
                <div class="wrap">
                    ...
                    <div class="cart">
                        ...
                        <?php foreach ($this->cart->contents() as $items): ?>
                            ...
                            // die();
                            //var_dump($items['options']['cgst']);
                           /*  echo $items['options']['sgst']."<br>";
							echo $items['options']['cgst']; */
                                ...
                                <div class="infoWrap d-flex align-items-center justify-content-between w-100">
                                    ...
                                    <div class="priceSection d-flex">
                                        <div class="prodTotal cartSection">
                                            ...
                                            <p class="fs-21 mb-0"><?php echo number_format($base_price,2); ?></p>
                                        </div>
                                        <div class="cartSection removeWrap ml-3">
											...
											<?php if(isset($state) && $items['options']['sgst']): ?>
                    ...
                    <div class="row justify-content-between mt-5">
                        ...
                        <div class="col-md-5 col-lg-5 text-right">
                            <ul class="subtotal list-unstyled">
                                    ...
                                    <span class="value" id="sub_total">
                                         ...
                                         <?php
										  $user_ip = $_SERVER['REMOTE_ADDR'];
											//echo "http://ip-api.com/json/$user_ip";
											// Use the ip-api.com service to get geolocation data based on the user's IP address
											$geolocation_data = file_get_contents("http://ip-api.com/json/$user_ip");
											// Check if data was successfully retrieved
											
												$geolocation_data = json_decode($geolocation_data);

												// Extract relevant geolocation information
												$user_country = $geolocation_data->country;
												$user_region = $geolocation_data->regionName;
												$user_city = $geolocation_data->city;

											
											
											
											    $total_tax_per = $cgst/100  + $sgst/100 + $igst/100;
											    $base_price = $items['price']/(1 + $total_tax_per);
											  $base_price=  number_format($base_price, 2, '.', '');
											  $sgst_tax = $sgst * $base_price * $target_con_rate / 100;
											  $cgst_tax = $cgst * $base_price * $target_con_rate / 100;
											  $igst_tax = $igst * $base_price  * $target_con_rate / 100;

											$total_tax = $sgst_tax + $cgst_tax + $igst_tax;
											
											$CI->_cart_contents['total_tax'] = $total_tax;
                                    // var_dump($sgst_tax);
                                    // var_dump($cgst_tax);
                                   // var_dump($total_tax);
                                    //var_dump(number_format($this->cart->total()));
                                    // die();
										$total = $this->cart->total(); // Total amount in the cart
                                        ...
                                        echo $currency . " " .number_format($sub_total,2); ?></span>
                                </li>
                                 <?php
                                ...
                                ?>
                                <li class="tax font-weight-200 fs-19 my-2" id="SGST">
								  <?php if($state == 'Punjab'): ?>
								...
								<?php if($state != 'Punjab'): ?>
                                <!-- <li class="tax font-weight-200 fs-19 my-2" id="IGST">
								<span class="label">IGST :</span>
								   ...
								   <span class="value">
                                        <?php
                                            ...
                                            // $total_tax = $cgst + $sgst + $igst;
                                            echo $currency . " " .number_format($igst_tax,2);
                            ...
                            <div class="site-footer mt-3">
								...
								if($state != 'Punjab'){
									  echo "Grand Total Amount (including IGST): " . number_format($totalAmount, 2) . " Rs.\n";
								}else{
									 echo "Grand Total Amount (including CGST, SGST): " . number_format($totalAmount, 2) . " Rs.\n";
								}
...
/* if ("geolocation" in navigator) {
            ...
            navigator.geolocation.getCurrentPosition(function(position) {
                ...
                // Use a reverse geocoding service to get the user's country
         fetch(`https://nominatim.openstreetmap.org/reverse?format=json&lat=${lat}&lon=${lon}`)
                    .then(response => response.json())
                    .then(data => {
                        
                        var userState = data.address.state; 
						...
						if(isUserInYourState)
						{
							...
							$('#SGST').html('<span class="label">SGST/CGST (18%):</span><span class="value">RS '+formattedtaxAmount+'</span>');
						}
						else
						{
							var taxRate = 18; // 18% tax rate
							...
							$('#sub_total').html('RS '.formattedSubTotal);
							$('#SGST').html('<span class="label">SGST/CGST :</span><span class="value">RS '+formattedtaxAmount+'</span>');
							$('#IGST').html('<span class="label">IGST :</span><span class="value">RS '+formattedtaxAmount+'</span>');
						}
                    })
                    .catch(error => {
