...
class Category extends MX_Controller
{
    ...
    {
		$id_or_tag_upper = strtoupper($cat_id); 
		if (stripos($id_or_tag_upper, 'TAG_') === 0) {
			
			$tag_prefix_length = strlen('TAG_');
			$tag = trim(substr($cat_id, $tag_prefix_length));
			$is_tag = true;
		}else{
			$is_tag = false;
		}
		//echo "Tag: $tags<br>";die;
        ...
        #
		if($is_tag === true){
			 $config["base_url"] = base_url('tag/TAG_' .remove_space($tag));
		}
        ...
        $content = $this->lcategory->category_product($cat_id,$links,$config["per_page"],$page,$price_range,$size, $brand,$rate,$filter_item,$tag);
