...
class Home extends MX_Controller
{
    function __construct()
    {
        ...
        $this->load->model('Product_Location');
    ...
    public function add_to_cart_details()
    {
        ...
        if ($product_id) {
			...
			if ($user_type == 6) {
			$product_details[0]->price = $product_details[0]->retail_price;
			} elseif ($user_type == 7) {
				$product_details[0]->price = $product_details[0]->wholesale_price;
			}
            ...
            // echo $product_details->cgst;
                $cgst = ($product_details->cgst * $final_price) / 100;
             

            //SGST product tax

        
                 $sgst = ($product_details->sgst * $final_price) / 100;
               

        
                $igst = ($product_details->igst * $final_price) / 100;
        ...
        if ($country_id) {
            ...
            if ($select_district_info) {
                ...
                foreach ($select_district_info as $district) {
                    $html .= "<option value=" . $district->name . " data-id=" . $district->id . ">$district->name</option>";
                }
                $html .= "</select>";
                echo $html;
            } 
    ...
    public function retrive_cities()
        {
            $state_id = $this->input->post('state_id',TRUE);

            if ($state_id) {
                $select_city_info = $this->Homes->select_city_info($state_id);

                $html = "";
                if ($select_city_info) {
                   
                    foreach ($select_city_info as $district) {
    ...
    public function submit_checkout()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $payment_method=$this->input->post('payment_method',TRUE);
            $diff_ship_adrs=$this->input->post('diff_ship_adrs',TRUE);
            $ordre_notes=$this->input->post('ordre_notes',TRUE);
            ...
            $this->session->set_userdata('ordre_notes', $ordre_notes);
            ...
            // var_dump($customer_existing_check);
            // die();
            if ($customer_existing_check) {
                if (1 == $diff_ship_adrs) {
                    ...
                    $billing = array(
                        ...
                        'city'                  =>$this->session->userdata('city') ?? $this->input->post('city'),
                ...
                $billing = array(
                    ...
                    'customer_mobile'       =>$this->session->userdata('customer_mobile'),
                    'customer_email'        =>$this->session->userdata('customer_email'),
                    'password'              =>md5("gef" .$this->input->post('ac_pass')),
                );
            ...
            $check_user_location = $this->Homes->check_user_loc($city);
			

            if (count($check_user_location) > 0) {
				//var_dump($check_user_location);die;
                 // If there are unavailable products, construct a message
                    $unavailableProductsMessage = "The following products are not available for city {$city}:\n";
                    foreach ($check_user_location as $product) {
                        $unavailableProductsMessage .= "- {$product}\n";
                    }
                    
                    // Set the message in the session
                    $this->session->set_userdata("error_message", $unavailableProductsMessage);
            ...
            if ($payment_method == 1) {
                //Order entry
                //echo $payment_method;die;
                $return_id = $this->Homes->order_entry($zip,$customer_id, $order_id);
                $return_order_id = $return_id['return_id'];
				$return_order_id_01 = $return_id['order_id'];
                //var_dump($return_order_id);die;
                $users = get_user_by_user_type(1);
				//var_dump($users);die;
                $notificationData = [
							'user_id' => $users->user_type,
							'type' => 'order_created',
							'data' => json_encode(['order_id' => $order_id,'order_no' => $return_order_id_01, 'content' => "You have a new order. Order ID: {$return_order_id_01}"]),
						];

						// Create a notification for each user with user type 1
						$this->NotificationModel->createNotification($notificationData);
              
                    $check_email =  $this->setmail($this->session->userdata('customer_email'));
                     if($check_email === true){
						 $this->cart->destroy();
						 redirect(base_url('track_my_order'));
					 }else{
						 redirect(base_url('checkout'));
					 }
...
//   // $appId='483451a04623719bf7c5a6e2054384';
     $appId='91065a82f2b154bf023aa86eb56019';

//   $post  = $this->Homes->paymentinfo('$payment_method');

                $Data['paymentinfo']  = $this->Homes->paymentinfo($payment_method);

//   $secretKey = $Data['paymentinfo']->r_pay_password;
  $secretKey ="32b6e90ff6197c3f4b9d69e1eb8d12ab33e1eb73";

  
// var_dump($secretKey);
// die();
     $returnUrl=base_url('cashfree_payment_success');
  $notifyUrl=base_url('cashfree_payment_success');
                // $postData['orderinfo'] = array();      
                // $postData['customerinfo'] = array();
                 
                $postData['appId']       = $appId;
                $postData['orderId']      = $order_id; 
                $postData['orderAmount']  = number_format($this->session->userdata('cart_total'),2,'.','');
                $postData['orderCurrency'] = 'INR'; 
                $postData['orderNote']='checkout';
                $postData['customerName']  =$this->session->userdata('first_name'). ' ' .$this->session->userdata('last_name');
                $postData['customerPhone']=$this->session->userdata('customer_mobile');
                $postData['customerEmail'] =$this->session->userdata('customer_email');
...
$this->load->view('accounting/components/payment-checkout',['postData'=>$postData,'signature'=>$signature,'url'=>$url]);

$query = get_user_by_user_type(1);
                //print_r($query->user_id);die;
                $notificationData = [
                        'user_id' => $query->user_id,
                        'type' => 'order_created',
                        'data' => json_encode(['order_id' =>$order_id,'content' => "You have a new order. Order ID: {$order_id}"]),
                    ];
                    $this->NotificationModel->createNotification($notificationData);
    ...
    // cashfree payment integrate  
    
    public function cashfree_payment_success()
{
...
//   die();
  $secretkey = "32b6e90ff6197c3f4b9d69e1eb8d12ab33e1eb73";
     ...
     $data = $order_id.$orderAmount.$referenceId.$txStatus.$paymentMode.$txMsg.$txTime;
     $hash_hmac = hash_hmac('sha256', $data, $secretkey, true) ;
     $computedSignature = base64_encode($hash_hmac);
        // $customer_id = 'GKGG6585YT78IT9';
        
          $customer_info = $this->db->select('customer_id')->from('customer_information')->where('customer_email', $this->session->userdata('customer_email'))->get()->row();
            $cust_id   = $customer_info->customer_id;
        // $customer = $this->session->userdata('customer_email');
...
//   die();
  if($signature != $computedSignature){
                    $this->session->set_userdata('error_message', 'Order Failed! Contact with Admin. Your CashFree transaction id is '.$referenceId);
  }
	...
	$return_order_id = $return_order_id['return_id']['order_id'];
      /* var_dump($return_order_id);
        die(); */
     $coupon_amnt = $this->session->userdata('coupon_amnt');
                if ($coupon_amnt > 0) {
  ...
  public function amazon_pay_success()
    ...
    */
    ...
    public function setmail($email, $file_path = null)
    {
        ...
        $this->email->set_newline("\r\n");
          $this->email->from('pwtramanuj@gmail.com', 'scetrxsjcipsustu');
        $this->email->to($email);
    ...
    public function track_my_order()
    {
        ...
        if($this->form_validation->run() == TRUE)
        {
            ...
            if($order_detail === false){
                $this->session->set_userdata("error_message", 'Record does not exists');
                redirect(base_url('track_my_order'));
                exit();
            }
	...
	public function filterType(){
		...
		$selected_filters = $this->input->post('query');
		//d($selected_filters);
		...
		if (!empty($selected_filters)) {
			...
			$this->db->join('filter_product fp', 'a.product_id = fp.product_id', 'left');
			$this->db->join('filter_types ft', 'ft.fil_type_id = fp.filter_type_id', 'left');
			$this->db->join('filter_items fi', 'fi.item_id = fp.filter_item_id', 'left');
			$this->db->where_in('ft.fil_type_id', $selected_filters);
			$this->db->or_where_in('fi.item_id', $selected_filters);
...
//print_r($this->db->last_query()); die;   
		...
		$products_str = '';
		 foreach ($data['products'] as $product) {
		     ...
		     $products_str .= $this->load->view('filter_product_listing', compact('product'), true);

		 
		}
		 $jsonData = json_encode($products_str);
...
//print_r($this->db->last_query()); die;   
		...
		$btn_more = '<button type="button" class="btn btn-outline-success float-end mt-4" id="load_more" data-id="'.$product->id.'">View All Premium Product <i class="fa-solid fa-angle-right"></i></button>';
		//$products_str .= '<div id="remove-row" class="some-class">' . $btn_more . '</div>';
			$response['last_product_id'] = $btn_more;
			$response['products_str'] = $products_str; 			
