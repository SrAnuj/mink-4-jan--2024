...
class Categories extends CI_Model
{
    ...
    //Category product
    public function category_product($cat_id, $per_page, $page, $price_range = null, $size = null, $brand=null, $rate=null,$filter_item=null,$tag = null)
    {
		//echo $tag;die;
		...
		if ($tag) {
			...
			$this->db->or_where("FIND_IN_SET('$tag', a.tag) >", 0);
		}
        if(empty($rate)){
    ...
    public function get_category_product($filter = [])
    {
        ...
        if(!empty($filter['product_name'])){
            $this->db->like('a.product_name', $filter['product_name'], 'both');
        }
    ...
    public function retrieve_category_product($product_name, $filter = [])
    {
		...
		// Add the join with filter_types table and filter_product table
		...
		// Search for products with a specific filter type name
		if (!empty($product_name)) {
			$this->db->or_like('ft.fil_type_name', $product_name, 'both');
			$this->db->or_like('fi.item_name', $product_name, 'both');
		}
		$this->db->group_by('a.product_id');
        $query = $this->db->get();
