...
class Homes extends CI_Model
{
    ...
    {
        parent::__construct();
       // $this->load->model('dashboard/Product_Location');
    }
...
//when any customer will purchase a product his information will be updated
 ...
 //Select state by country
    ...
    public function select_ship_state_city($stateID)
    {
...
//send sms after order completed
    ...
    //Order entry
public function check_user_loc($city) {
    $this->load->model('dashboard/Product_Location');
    $cart_contents = $this->cart->contents();
    $unavailableProducts = array(); // Initialize an array to store unavailable products

    foreach ($cart_contents as $item) {
        $product_id = $item['product_id'];
        $check_product_info = $this->Product_Location->isProductAvailableForZipCode($product_id, $city);

        // Assuming is_perishable is a boolean field
        $is_perishable = $this->Product_Location->getPerishableStatusFromDB($product_id);
		
        if ($check_product_info === false && $is_perishable == true) {
            // Product is not available for the specified city or not perishable
    ...
    public function order_entry($zip,$customer_id, $order_id, $transdata = [])
    {
...
//             exit;
        ...
        if ($payment_method == 1) {
            ...
            $data = array(
                ...
                'order_notes' => $this->session->userdata('ordre_notes'),
        ...
        // echo "lklklk";
          $data = array(
            ...
            'delivery_id' => $this->session->userdata('method_id') ?? 1,
            'order_id' => $order_id,
