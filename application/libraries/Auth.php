...
// die();
        if ($result) {
            ...
            // die();
                        ...
                        $sales_target = $CI->Users->salesTarget ($result[0]['user_id']);

            // var_dump($sales_target[0]->sales_target);
            // die();
            ...
            $user_data = array(
                ...
                'sales_target' => $sales_target[0]->sales_target,
...
// var_dump($user_data);
  ...
  function check_delivery_auth($url = '')
    {
        if ($url == '') {
            $url = base_url() . 'admin';
        }
        $CI =& get_instance();
        if ((!$this->is_logged()) || (!$this->is_delivery())) {
            $this->logout();
            $error = display('you_are_not_authorised');
            $CI->session->set_userdata(array('error_message' => $error));
            redirect($url, 'refresh');
        ...
        function check_admin_delivery_auth($url = '')
    {
        if ($url == '') {
            $url = base_url() . 'admin';
        }
        $CI =& get_instance();
        if ((!$this->is_logged()) && (!$this->is_admin()) && (!$this->is_delivery())) {
            $this->logout();
            $error = display('you_are_not_authorised');
            $CI->session->set_userdata(array('error_message' => $error));
            redirect($url, 'refresh');
