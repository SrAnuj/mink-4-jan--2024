...
if (!function_exists('getNotifications')) {
    function getNotifications($user_id) {
        $CI = &get_instance(); // Get CodeIgniter's instance
        $CI->db->select('*');
        $CI->db->from('notifications');
        $CI->db->where('user_id', $user_id);
        $CI->db->where('read_at', null); // Adjust the user_id condition as needed
        $CI->db->order_by('created_at', 'desc');
         $CI->db->limit(3); // Order by the creation date in descending order
        $notifications = $CI->db->get()->result();
        return $notifications;
    }
}
if (!function_exists('getCountNotifications')) {
    function getCountNotifications($user_id) {
        $CI = &get_instance(); // Get CodeIgniter's instance
        $CI->db->select('*');
        $CI->db->from('notifications');
        $CI->db->where('user_id', $user_id);
...
if (!function_exists('get_user_by_user_type')) {
    function get_user_by_user_type($user_type) {
        $CI = &get_instance(); // Get the CodeIgniter instance

		// Query the database to fetch the second user record with the specified user type and join with the "users" table
		$CI->db->select('*'); // Select all columns, you can specify specific columns if needed
		$CI->db->from('user_login');
		$CI->db->where('user_login.user_type', $user_type);
		$CI->db->order_by('user_login.user_id', 'desc'); // Order by 'id' in descending order
		$CI->db->limit(1, 1); // Limit to one record starting from the second record

		// Join with the "users" table based on a common column (e.g., "user_id")
		$CI->db->join('users', 'user_login.user_id = users.user_id', 'inner'); // Adjust the join type as needed (e.g., inner, left, right)
...
if (!function_exists('humanDate')) {
    function humanDate($timestamp) {
        ...
        if ($timeDiff < 60) {
            ...
            return date('F j, Y', $timestamp); // Format for older dates (e.g., September 1, 2023)
        }
    }
}
