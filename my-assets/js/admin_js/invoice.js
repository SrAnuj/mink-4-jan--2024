...
function addEditInvoiceItem(t) {
    ...
    if (count == limits) alert("You have reached the limit of adding " + count + " inputs");
        ...
        var a = "product_name" + count,
            e = document.createElement("tr");
        e.innerHTML = "<td><input type='text' name='product_name' placeholder='Product Name' onkeypress='invoice_productList(" + count + ");' class='form-control productSelection' required='' id='product_name" + count + "' ><input type='hidden' class='autocomplete_hidden_value product_id_" + count + "' name='product_id[]' id='SchoolHiddenId'/></td><td><input type='text' name='available_quantity[]' id='' class='form-control text-right available_quantity_" + count + "' placeholder='0' readonly='' /></td><td><input type='text' class='form-control text-right unit_" + count + "' placeholder='None' readonly='' /></td><td><input type='number' name='product_quantity[]' onkeyup='quantity_calculate(" + count + ");' onchange='quantity_calculate(" + count + ");' id='total_qntt_" + count + "' class='form-control text-right' value='1' min='1' /></td><td><input type='number' name='product_rate[]' onkeyup='quantity_calculate(" + count + ");' onchange='quantity_calculate(" + count + ");' placeholder='0.00' min='0' id='price_item_" + count + "' class='price_item" + count + " form-control text-right' required='' /></td><td><input type='number' name='discount[]' onkeyup='quantity_calculate(" + count + ");' onchange='quantity_calculate(" + count + ");' id='discount_" + count + "' class='form-control text-right' placeholder='0.00' min='0' /></td><td><input class='total_price form-control text-right' type='text' name='total_price[]' id='total_price_" + count + "' placeholder='0.00' tabindex='' readonly='readonly' /></td><td><input type='hidden' id='plan_"+ count +"' class='plan'/></td><td><input type='hidden' id='cgst_" + count + "' class='cgst'/> <input type='hidden' id='sgst_" + count + "' class='sgst'/><input type='hidden' id='igst_" + count + "' class='igst'/><input type='hidden' id='total_cgst_" + count + "' class='total_cgst' name='cgst[]' /><input type='hidden' id='total_sgst_" + count + "' class='total_sgst' name='sgst[]'/><input type='hidden' id='total_igst_" + count + "' class='total_igst' name='igst[]'/><input type='hidden' name='variant_id[]' id='variant_" + count + "'><input type='hidden' id='total_discount_" + count + "' /><input type='hidden' id='all_discount_" + count + "' class='total_discount'/><button style='text-align: right;' class='btn btn-danger' type='button' value='Delete' onclick='deleteRow(this)'>Delete</button></td>", document.getElementById(t).appendChild(e), document.getElementById(a).focus(), count++
    }
}
...
function quantity_calculate(item) {
    ...
    var cgst_tax  =net_price*cgst/100 ;
    var sgst_tax  =  net_price*sgst/100;
    var igst_tax  =  net_price*igst/100;
...
$("#total_discount_ammount").on("blur", function() {
    calculateSum();
});
//Calculate all summation
function calculateSum(discountType = null) {
    var cgst = 0;
    var sgst = 0;
    var igst = 0;
    var e = 0;
    ...
    var grand_total = total_price.toFixed(2);

    ser_chg = $("#service_charge").val();
    shipping_charge = $("#shipping_charge").val() ? $("#shipping_charge").val() : 0;

    // Apply the discount based on the selected discount type if provided
    var invoiceDiscountValue = $("#invoice_discount").val();
    if (invoiceDiscountValue !== null && invoiceDiscountValue !== "") {
        inv_dis = parseFloat(invoiceDiscountValue);

        // Apply the discount based on the selected discount type if provided
        if (discountType !== null) {
            if (discountType === "fixed") {
                f = Math.min(inv_dis, grand_total).toFixed(2);
            } else if (discountType === "percentage") {
                f = Math.min((grand_total * (inv_dis / 100)), grand_total).toFixed(2);
            }
        }
    }

    sum = +cgst + +sgst + +igst + +grand_total - f - inv_dis + +ser_chg + +shipping_charge;
    $("#grandTotal").val(Math.max(sum, 0).toFixed(2)); // Ensure the result is not negative
    $(".total_bill").text(Math.max(sum, 0).toFixed(2));
